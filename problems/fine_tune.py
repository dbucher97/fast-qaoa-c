...
SHIFT = 0.4
INTERPOLATORS = {
    M: get_indicator_interpolator(M, 4, SHIFT) for M in [2, 4, 6, 8, 12, 16]
}
    ...
    for M, interp in INTERPOLATORS.items():
        ...
        for _, row in data.sort_values(by="depth").iterrows():
            ...
            res = optimize_qaoa_lbfgs(
                f, cost, row.betas, row.gammas, constr=sgn, maxiter=10
            )
...
df = pd.concat([df, pd.DataFrame(collected)]).reset_index()
